...
import "../interfaces/ICrossHop.sol";
import "../interfaces/IWETH9.sol";

/**
 * @title L1BridgeZap
 * @notice This contract is responsible for handling user Zaps into the CrossHop contract, through the CrossHop Swap contracts. It does so
 ...
 * This is done in hopes of automating portions of the bridge user experience to users, while keeping the CrossHop contract logic small.
 *
 * @dev This contract should be deployed with a base Swap.sol address and a CrossHop.sol address, otherwise, it will not function.
 */
contract L1BridgeZap {
    ...
    constructor(
        ...
        baseSwap = _baseSwap;
        CrossHop = _CrossHop;
        IERC20(_wethAddress).safeIncreaseAllowance(address(_CrossHop), MAX_UINT256);
    ...
    /**
     * @notice Wraps CrossHop deposit() function to make it compatible w/ ETH -> WETH conversions
    ...
    function depositETH(
        ...
        CrossHop.deposit(to, chainId, IERC20(WETH_ADDRESS), amount);
    }

    /**
     * @notice Wraps CrossHop depositAndSwap() function to make it compatible w/ ETH -> WETH conversions
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
     * @param amount Amount in native token decimals to transfer cross-chain pre-fees
     ...
     * @param isDeposit whether this is a deposit or a withdrawal
     * @return token amount the user will receive
     */
    function calculateTokenAmount(uint256[] calldata amounts, bool isDeposit) external view virtual returns (uint256) {
        return baseSwap.calculateTokenAmount(amounts, isDeposit);
    }
    ...
    function zapAndDeposit(
        ...
        if (token.allowance(address(this), address(CrossHop)) < liqAdded) {
            token.safeApprove(address(CrossHop), MAX_UINT256);
        }
        CrossHop.deposit(to, chainId, token, liqAdded);
    }
    ...
    function zapAndDepositAndSwap(
        ...
        CrossHop.depositAndSwap(to, chainId, token, liqAdded, tokenIndexFrom, tokenIndexTo, minDy, swapDeadline);
    }

    /**
     * @notice Wraps CrossHop deposit() function
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
     * @param token ERC20 compatible token to deposit into the bridge
     * @param amount Amount in native token decimals to transfer cross-chain pre-fees
    ...
    function deposit(
        ...
        if (token.allowance(address(this), address(CrossHop)) < amount) {
            token.safeApprove(address(CrossHop), MAX_UINT256);
        }
        CrossHop.deposit(to, chainId, token, amount);
    }

    /**
     * @notice Wraps CrossHop depositAndSwap() function
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
     * @param token ERC20 compatible token to deposit into the bridge
     * @param amount Amount in native token decimals to transfer cross-chain pre-fees
    ...
    function depositAndSwap(
        ...
        CrossHop.depositAndSwap(to, chainId, token, amount, tokenIndexFrom, tokenIndexTo, minDy, deadline);
    }

    /**
     * @notice Wraps CrossHop redeem() function
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
    ...
    function redeem(
        ...
        CrossHop.redeem(to, chainId, token, amount);
    }

    /**
     * @notice Wraps redeemAndSwap on CrossHop.sol
    ...
    function redeemAndSwap(
        ...
        CrossHop.redeemAndSwap(to, chainId, token, amount, tokenIndexFrom, tokenIndexTo, minDy, deadline);
    }

    /**
     * @notice Wraps redeemAndRemove on CrossHop
     * Relays to nodes that (typically) a wrapped synAsset ERC20 token has been burned and the underlying needs to be redeeemed on the native chain. This function indicates to the nodes that they should attempt to redeem the LP token for the underlying assets (E.g "swap" out of the LP token)
     * @param to address on other chain to redeem underlying assets to
     * @param chainId which underlying chain to bridge assets onto
     * @param token ERC20 compatible token to deposit into the bridge
    ...
    function redeemAndRemove(
        ...
        CrossHop.redeemAndRemove(to, chainId, token, amount, liqTokenIndex, liqMinAmount, liqDeadline);
    }

    /**
     * @notice Wraps CrossHop redeemv2() function
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
     * @param token ERC20 compatible token to redeem into the bridge
     * @param amount Amount in native token decimals to transfer cross-chain pre-fees
    ...
    function redeemv2(
        ...
        CrossHop.redeemv2(to, chainId, token, amount);
    }
}
