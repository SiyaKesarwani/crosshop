...
contract L2BridgeZap {
    ...
    function swapAndRedeem(
        ...
        if (token.allowance(address(this), address(CrossHop)) < swappedAmount) {
            token.safeApprove(address(CrossHop), MAX_UINT256);
        }
        CrossHop.redeem(to, chainId, token, swappedAmount);
    }
    ...
    function swapAndRedeemAndRemove(
        ...
        CrossHop.redeemAndRemove(to, chainId, token, swappedAmount, liqTokenIndex, liqMinAmount, liqDeadline);
    }

    /**
     * @notice wraps CrossHopBridge redeem()
    ...
    /**
     * @notice Wraps CrossHopBridge deposit() function to make it compatible w/ ETH -> WETH conversions
    ...
    /**
     * @notice Wraps CrossHopBridge depositAndSwap() function to make it compatible w/ ETH -> WETH conversions
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
     * @param amount Amount in native token decimals to transfer cross-chain pre-fees
    ...
    /**
     * @notice Wraps redeemAndSwap on CrossHopBridge.sol
    ...
    /**
     * @notice Wraps redeemAndRemove on CrossHopBridge
     * Relays to nodes that (typically) a wrapped synAsset ERC20 token has been burned and the underlying needs to be redeeemed on the native chain. This function indicates to the nodes that they should attempt to redeem the LP token for the underlying assets (E.g "swap" out of the LP token)
     * @param to address on other chain to redeem underlying assets to
     * @param chainId which underlying chain to bridge assets onto
     * @param token ERC20 compatible token to deposit into the bridge
    ...
    /**
     * @notice Wraps CrossHopBridge redeemv2() function
     * @param to address on other chain to bridge assets to
     * @param chainId which chain to bridge assets onto
