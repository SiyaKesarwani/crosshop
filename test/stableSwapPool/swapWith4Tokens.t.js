...
describe("Swap with 4 tokens", () => {
  ...
  async function setupTest() {
    ...
    cUSD = (await erc20Factory.deploy("cUSD", "cUSD", "18"));

    TOKENS.push(DAI, USDC, USDT, cUSD);
    ...
    await asyncForEach(
      ...
      async (address) => {
        ...
        await cUSD.mint(address, String(1e20));
      }
    );
    ...
    await swap.initialize(
      [DAI.address, USDC.address, USDT.address, cUSD.address],
      [18, 6, 6, 18],
      LP_TOKEN_NAME,
      LP_TOKEN_SYMBOL,
      INITIAL_A_VALUE,
    ...
    await asyncForEach([owner, user1, user2, attacker], async (signer) => {
      ...
      await cUSD.connect(signer).approve(swap.address, MAX_UINT256);
    });
  ...
  describe("withdrawAdminFees", () => {
    ...
    it("Succeeds when there are no fees withdrawn", async () => {
      ...
      const balancesBefore = await getUserTokenBalances(owner, [DAI, USDC, USDT, cUSD]);

      await swap.withdrawAdminFees();

      const balancesAfter = await getUserTokenBalances(owner, [DAI, USDC, USDT, cUSD]);
    ...
    it("Withdrawing admin fees has no impact on users' withdrawal", async () => {
      ...
      const balancesBefore = await getUserTokenBalances(user1, [DAI, USDC, USDT, cUSD]);
      ...
      const balancesAfter = await getUserTokenBalances(user1, [DAI, USDC, USDT, cUSD]);
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 16e6 of USDC to cUSD, causing massive imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(16e6), 0, MAX_UINT256);
            const cUSDOutput = (await getUserTokenBalance(attacker, cUSD)).sub(
              initialAttackerBalances[3]
            );

            // First trade results in 15.87e18 of cUSD
            expect(cUSDOutput).to.be.eq("15873636661935380627");

            // Pool is imbalanced! Now trades from cUSD -> USDC may be profitable in small sizes
            // USDC balance in the pool : 66e6
            // cUSD balance in the pool : 34.13e18
            ...
            // Trade cUSD to USDC, taking advantage of the imbalance and change of A
            const balanceBefore = await getUserTokenBalance(attacker, USDC);
            await swap.connect(attacker).swap(3, 1, cUSDOutput, 0, MAX_UINT256);
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Trade cUSD to USDC, taking advantage of the imbalance and sudden change of A
            const balanceBefore = await getUserTokenBalance(attacker, USDC);
            await swap.connect(attacker).swap(3, 1, cUSDOutput, 0, MAX_UINT256);
            const USDCOutput = (await getUserTokenBalance(attacker, USDC)).sub(balanceBefore);
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swapping 25e6 of USDC to cUSD, resolving imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const cUSDOutput = (await getUserTokenBalance(attacker, cUSD)).sub(
              initialAttackerBalances[3]
            );

            // First trade results in 25.14e18 of cUSD
            // Because the pool was imbalanced in the beginning, this trade results in more than 25e18 cUSD
            expect(cUSDOutput).to.be.eq("25140480043410581418");

            // Pool is now almost balanced!
            // USDC balance in the pool : 75.00e6
            // cUSD balance in the pool : 74.86e18
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Swap 25e6 of USDC to cUSD, resolving the imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const cUSDOutput = (await getUserTokenBalance(attacker, cUSD)).sub(
              initialAttackerBalances[3]
            );
    ...
    describe("Check for attacks while A is ramping downwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 25e6 of USDC to cUSD, resolving imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const cUSDOutput = (await getUserTokenBalance(attacker, cUSD)).sub(
              initialAttackerBalances[3]
            );
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // First trade results in 25.14e18 of cUSD
            // Because the pool was imbalanced in the beginning, this trade results in more than 1e18 cUSD
            expect(cUSDOutput).to.be.eq("25140480043410581418");
