...
describe('Swap with 2 tokens', async function () {
    let swap;
    let swapStorage;
    let swapToken;
    let signers;
    ...
    async function setupTest() {
      ...
      await asyncForEach([owner, user1, user2], async (signer) => {
        ...
        await firstToken.mint(address, 1e20.toString());
        await secondToken.mint(address, 1e20.toString());
      });
      ...
      swapToken = (await ethers.getContractAt("LPToken", swapStorage.lpToken));
      ...
      await swap.addLiquidity([1e18.toString(), 1e18.toString()], 0, MAX_UINT256);
  
      expect(await firstToken.balanceOf(swap.address)).to.eq(1e18.toString());
      expect(await secondToken.balanceOf(swap.address)).to.eq(1e18.toString());
    };
    ...
    describe("swapStorage", () => {
      describe("lpToken", async () => {
        ...
        it("Returns true after successfully calling transferFrom", async () => {
          ...
          await swap.connect(user1).addLiquidity([2e18.toString(), 1e16.toString()], 0, MAX_UINT256);
    ...
    describe("getTokenBalance", () => {
      it("Returns correct balances of pooled tokens", async () => {
        expect(await swap.getTokenBalance(0)).to.eq(1e18.toString());
        expect(await swap.getTokenBalance(1)).to.eq(1e18.toString());
      });
    ...
    describe("addLiquidity", () => {
      it("Reverts when contract is paused", async () => {
        ...
        await expect(swap.connect(user1).addLiquidity([1e18.toString(), 3e18.toString()], 0, MAX_UINT256))
          .to.be.reverted;
  
        // unpause
        await swap.unpause();
  
        await swap.connect(user1).addLiquidity([1e18.toString(), 3e18.toString()], 0, MAX_UINT256);
      ...
      it("Reverts with 'Amounts must match pooled tokens'", async () => {
        await expect(
          swap.connect(user1).addLiquidity([1e16.toString()], 0, MAX_UINT256)
        ).to.be.revertedWith("Amounts must match pooled tokens");
      });
  
      it("Reverts with 'Cannot withdraw more than available'", async () => {
        await expect(
          swap.connect(user1).calculateTokenAmount([MAX_UINT256, 3e18.toString()], false)
        ).to.be.revertedWith("Cannot withdraw more than available");
      });
  
      it("Reverts with 'Must supply all tokens in pool'", async () => {
        swapToken.approve(swap.address, 2e18.toString());
        await swap.removeLiquidity(2e18.toString(), [0, 0], MAX_UINT256);
        await expect(
          swap.connect(user1).addLiquidity([0, 3e18.toString()], MAX_UINT256, MAX_UINT256)
      ...
      it("Succeeds with expected output amount of pool tokens", async () => {
        const calculatedPoolTokenAmount = await swap
          .connect(user1)
          .calculateTokenAmount([1e18.toString(), 3e18.toString()], true);
      ...
      it("Succeeds with correctly updated tokenBalance after imbalanced deposit", async () => {
        ...
        expect(await swap.getTokenBalance(0)).to.eq(BigNumber.from(2e18.toString()));
        expect(await swap.getTokenBalance(1)).to.eq(BigNumber.from(4e18.toString()));
      });
  
      it("Returns correct minted lpToken amount", async () => {
        await firstToken.mint(user1.address, 1e20.toString());
        await secondToken.mint(user1.address, 1e20.toString());
  
        await swap.connect(user1).addLiquidity([1e18.toString(), 2e18.toString()], 0, MAX_UINT256);

        const actualPoolTokenAmountReceived = await swapToken.balanceOf(user1Address);
        expect(actualPoolTokenAmountReceived).to.eq(BigNumber.from("2997459774673651937"))
      });
      ...
      it("Reverts when minToMint is not reached due to front running", async () => {
        ...
        await swap.addLiquidity([1e18.toString(), 3e18.toString()], 0, MAX_UINT256);
      ...
      it("Reverts when block is mined after deadline", async () => {
        ...
        await expect(
          swap.connect(user1).addLiquidity([2e18.toString(), 1e16.toString()], 0, currentTimestamp + 60 * 5)
      ...
      it("Emits addLiquidity event", async () => {
        const calculatedLPTokenAmount = await swap
          .connect(user1)
          .calculateTokenAmount([2e18.toString(), 1e16.toString()], true);
    ...
    describe("removeLiquidity", () => {
      ...
      it("Reverts with 'minAmounts must match poolTokens'", async () => {
        await expect(swap.removeLiquidity(2e18.toString(), [0], MAX_UINT256)).to.be.revertedWith(
      ...
      it("Returns correct amounts of received tokens", async () => {
        ...
        await swap.connect(user1).addLiquidity([1e18.toString(), 2e18.toString()], 0, MAX_UINT256);
        const tokenBalance = await swapToken.balanceOf(user1Address);
  
        await swap.connect(user1).removeLiquidity(tokenBalance, [0, 0], MAX_UINT256);
      });
      ...
      it("Reverts when minAmounts of underlying tokens are not reached due to front running", async () => {
        ...
        await swap.connect(user2).addLiquidity([1e16.toString(), 2e18.toString()], 0, MAX_UINT256);
    ...
    describe("removeLiquidityImbalance", () => {
      it("Reverts when contract is paused", async () => {
        ...
        await expect(
          swap.removeLiquidityImbalance([1e18.toString(), 1e16.toString()], MAX_UINT256, MAX_UINT256)
        ).to.be.reverted;
  
        await expect(
          swap
            .connect(user1)
            .removeLiquidityImbalance([1e18.toString(), 1e16.toString()], MAX_UINT256, MAX_UINT256)
        ).to.be.reverted;
      });
  
      it("Reverts with 'Amounts should match pool tokens'", async () => {
        await expect(
          swap.removeLiquidityImbalance([1e18.toString()], MAX_UINT256, MAX_UINT256)
      ...
      it("Succeeds with calculated max amount of pool token to be burned (±0.1%)", async () => {
        ...
        expect(firstTokenBalanceAfter.sub(firstTokenBalanceBefore)).to.eq(1e18.toString());
        expect(secondTokenBalanceAfter.sub(secondTokenBalanceBefore)).to.eq(1e16.toString());
  
        // Check the actual burned pool token amount
        const actualPoolTokenBurned = poolTokenBalanceBefore.sub(poolTokenBalanceAfter);
  
        expect(actualPoolTokenBurned).to.eq(BigNumber.from("1000934178112841889"));
      ...
      it("Reverts when minAmounts of underlying tokens are not reached due to front running", async () => {
        ...
        // User 2 adds liquidity, which leads to change in balance of underlying tokens
        await swap.connect(user2).addLiquidity([1e16.toString(), 1e20.toString()], 0, MAX_UINT256);
      ...
      it("Emits RemoveLiquidityImbalance event", async () => {
        ...
        await expect(
          swap
            .connect(user1)
            .removeLiquidityImbalance([1e18.toString(), 1e16.toString()], currentUser1Balance, MAX_UINT256)
    ...
    describe("removeLiquidityOneToken", () => {
      it("Reverts when contract is paused.", async () => {
        ...
        await expect(swap.removeLiquidityOneToken(2e18.toString(), 0, 0, MAX_UINT256)).to.be.reverted;
        await expect(
      ...
      it("Returns correct amount of received token", async () => {
        await firstToken.mint(user1.address, 1e20.toString());
        await secondToken.mint(user1.address, 1e20.toString());
        await swap.addLiquidity([1e18.toString(), 2e18.toString()], 0, MAX_UINT256);
        await swap.removeLiquidity(2e18.toString(), [0, 0], MAX_UINT256);
      });
    ...
    describe("swap", () => {
      it("Reverts when contract is paused", async () => {
        ...
        await expect(swap.connect(user1).swap(0, 1, 1e16.toString(), 0, MAX_UINT256)).to.be.reverted;
      });
  
      it("Reverts with 'Token index out of range'", async () => {
        await expect(swap.calculateSwap(0, 9, 1e17.toString())).to.be.revertedWith(
          "Token index out of range"
        );
      });
      ...
      it("Succeeds with expected swap amounts", async () => {
        ...
        const calculatedSwapReturn = await swap.calculateSwap(0, 1, 1e17.toString());
        ...
        await swap.connect(user1).swap(0, 1, 1e17.toString(), calculatedSwapReturn, MAX_UINT256);
        ...
        expect(tokenFromBalanceBefore.sub(tokenFromBalanceAfter)).to.eq(BigNumber.from(1e17.toString()));
      ...
      it("Reverts when minDy (minimum amount token to receive) is not reached due to front running", async () => {
        ...
        await swap.connect(user2).swap(0, 1, 1e17.toString(), 0, MAX_UINT256);
  
        // User 1 initiates swap
        await expect(swap.connect(user1).swap(0, 1, 1e17.toString(), calculatedSwapReturn, MAX_UINT256))
          .to.be.reverted;
      });
      ...
      it("Succeeds when using lower minDy even when transaction is front-ran", async () => {
        ...
        await swap
          .connect(user1)
          .swap(0, 1, 1e17.toString(), calculatedSwapReturnWithNegativeSlippage, MAX_UINT256);
      ...
      it("Returns correct amount of received token", async () => {
        ...
        await secondToken.mint(user1.address, 1e20.toString());
        await swap.addLiquidity([1e18.toString(), 2e18.toString()], 0, MAX_UINT256);
        await swap.swap(0, 1, 1e18.toString(), 0, MAX_UINT256);
      });
      ...
      it("Reverts when block is mined after deadline", async () => {
        ...
        await expect(
          swap.connect(user1).swap(0, 1, 1e17.toString(), 0, currentTimestamp + 60 * 5)
        ).to.be.revertedWith("Deadline not met");
      });
  
      it("Emits TokenSwap event", async () => {
        // User 1 initiates swap
        await expect(swap.connect(user1).swap(0, 1, 1e17.toString(), 0, MAX_UINT256)).to.emit(
          swap,
          "TokenSwap"
        );
      });
    ...
    describe("getVirtualPrice", () => {
      ...
      it("Returns expected values after imbalanced withdrawal", async () => {
        await swap.connect(user1).addLiquidity([1e18.toString(), 1e18.toString()], 0, MAX_UINT256);
        await swap.connect(user2).addLiquidity([1e18.toString(), 1e18.toString()], 0, MAX_UINT256);
        expect(await swap.getVirtualPrice()).to.eq(1e18.toString());
  
        await swapToken.connect(user1).approve(swap.address, 2e18.toString());
        await swap
          .connect(user1)
          .removeLiquidityImbalance([1e18.toString(), 0], 2e18.toString(), MAX_UINT256);
  
        expect(await swap.getVirtualPrice()).to.eq(BigNumber.from("1000100094088440633"));
  
        await swapToken.connect(user2).approve(swap.address, 2e18.toString());
        await swap
          .connect(user2)
          .removeLiquidityImbalance([0, 1e18.toString()], 2e18.toString(), MAX_UINT256);
      ...
      it("Value is unchanged after balanced deposits", async () => {
        ...
        await swap.connect(user2).addLiquidity([2e18.toString(), BigNumber.from("0")], 0, MAX_UINT256);
        ...
        await swap.connect(user2).addLiquidity([2e18.toString(), 1e18.toString()], 0, MAX_UINT256);
        expect(await swap.getVirtualPrice()).to.eq(BigNumber.from("1000167146429977312"));
      });
  
      it("Value is unchanged after balanced withdrawals", async () => {
        await swap.connect(user1).addLiquidity([1e18.toString(), 1e18.toString()], 0, MAX_UINT256);
        await swapToken.connect(user1).approve(swap.address, 1e18.toString());
        await swap.connect(user1).removeLiquidity(1e18.toString(), ["0", "0"], MAX_UINT256);
        expect(await swap.getVirtualPrice()).to.eq(1e18.toString());
      });
    });
    ...
    describe("getAdminBalance", () => {
      ...
      it("Returns expected amounts after swaps when adminFee is higher than 0", async () => {
        ...
        expect(await swap.getAdminBalance(0)).to.eq(0);
        expect(await swap.getAdminBalance(1)).to.eq(BigNumber.from("998024139765"));
        ...
        expect(await swap.getAdminBalance(0)).to.eq(BigNumber.from("1001973776101"));
        expect(await swap.getAdminBalance(1)).to.eq(BigNumber.from("998024139765"));
      });
    });
    ...
    describe("withdrawAdminFees", () => {
      ...
      it("Succeeds with expected amount of fees withdrawn", async () => {
        ...
        expect(firstTokenAfter.sub(firstTokenBefore)).to.eq(BigNumber.from("1001973776101"));
        expect(secondTokenAfter.sub(secondTokenBefore)).to.eq(BigNumber.from("998024139765"));
      });
      ...
      it("Withdrawing admin fees has no impact on users' withdrawal", async () => {
        ...
        for (let i = 0; i < 10; i++) {
          await swap.connect(user2).swap(0, 1, 1e17.toString(), 0, MAX_UINT256);
          await swap.connect(user2).swap(1, 0, 1e17.toString(), 0, MAX_UINT256);
        }
    ...
    describe("rampA", () => {
      ...
      it("Succeeds to ramp upwards", async () => {
        ...
        await swap.addLiquidity([1e18.toString(), 0], 0, MAX_UINT256);
    ...
    describe("Check for timestamp manipulations", () => {
      ...
      describe("Check for attacks while A is ramping upwards", () => {
        ...
        beforeEach(async () => {
          ...
          expect(initialAttackerBalances[0]).to.be.eq(1e20.toString());
          expect(initialAttackerBalances[1]).to.be.eq(1e20.toString());
          ...
          expect(initialPoolBalances[0]).to.be.eq(1e18.toString());
          expect(initialPoolBalances[1]).to.be.eq(1e18.toString());
        });
        ...
        describe(
          ...
          () => {
            it("Attack fails with 900 seconds between blocks", async () => {
              // Swap 1e18.toString() of firstToken to secondToken, causing massive imbalance in the pool
              await swap.connect(attacker).swap(0, 1, 1e18.toString(), 0, MAX_UINT256);
              ...
              expect(await swap.getTokenBalance(0)).to.be.eq(2e18.toString());
              ...
              // If firstTokenOutput > 1e18.toString(), the malicious user leaves with more firstToken than the start.
        ...
        describe(
          ...
          () => {
            beforeEach(async () => {
              ...
              await swap
                .connect(user2)
                .addLiquidity([BigNumber.from("0"), 2e18.toString()], 0, (await getCurrentBlockTimestamp()) + 60);
  
              // Check current pool balances
              initialPoolBalances = [await swap.getTokenBalance(0), await swap.getTokenBalance(1)];
              expect(initialPoolBalances[0]).to.be.eq(1e18.toString());
              expect(initialPoolBalances[1]).to.be.eq(3e18.toString());
            });
  
            it("Attack fails with 900 seconds between blocks", async () => {
              // Swap 1e18.toString() of firstToken to secondToken, resolving imbalance in the pool
              await swap.connect(attacker).swap(0, 1, 1e18.toString(), 0, MAX_UINT256);
              const secondTokenOutput = (await getUserTokenBalance(attacker, secondToken)).sub(
                initialAttackerBalances[1]
              );
              ...
              // Because the pool was imbalanced in the beginning, this trade results in more than 1e18.toString() secondToken
              ...
              // If firstTokenOutput > 1e18.toString(), the attacker leaves with more firstToken than the start.
            ...
            it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
              ...
              // Swap 1e18.toString() of firstToken to secondToken, resolving the imbalance in the pool
              await swap.connect(attacker).swap(0, 1, 1e18.toString(), 0, MAX_UINT256);
              const secondTokenOutput = (await getUserTokenBalance(attacker, secondToken)).sub(
                initialAttackerBalances[1]
              );
